...
class EpiDatabase
{
  ...
  public function all($sql = false, $params = array(), $limit = 0)
  {
    $this->init();
    try
    {
      $sth = $this->prepare($sql, $params, $limit);
  ...
  private function prepare($sql, $params = array(), $limit = 0)
  {
    try
    {
      if($limit > 0) {
      ...
      if($limit > 0) {
        $sth->bindParam(':epiLimit', $limit, PDO::PARAM_INT);
